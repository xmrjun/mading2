...
### 🎯 多策略模式
- **单策略模式**：专注单一交易策略，适合明确市场方向
- **双策略模式**：智能切换两种策略，适应不同市场波动
...
**智能切换逻辑**：
- 🔍 **波动性分析**：实时分析市场波动程度
- 🎯 **策略选择**：小波动选择策略1，大波动选择策略2
- ⚖️ **冲突避免**：两策略间1分钟间隔防冲突
- 🛡️ **风险控制**：全局风险控制和每日限制

### 模式3：策略1单独运行 🎯
**适用场景**：想用高频策略但不要双策略复杂性
```json
// dual_strategy_config.json
{
  "enableDualStrategy": false,  // ← 关键设置
  // 系统将只运行strategy1，使用5000 USDC
}
```
...
graph TD
    ...
    B -->|存在| C{enableDualStrategy?}
    B -->|不存在| F[使用 backpack_trading_config.json]
    C -->|true| D[双策略模式]
    C -->|false| E[策略1单独运行]
    F --> G[单策略模式]
    
    D --> H[同时运行策略1+策略2]
    E --> I[只运行策略1，5000 USDC]
    G --> J[运行激进策略，5000 USDC]
```

### 📊 配置对比表

| 配置项 | 单策略模式 | 双策略模式 | 策略1单独 |
|-------|-----------|-----------|----------|
| **配置文件** | `backpack_trading_config.json` | `dual_strategy_config.json` | `dual_strategy_config.json` |
| **启用设置** | - | `enableDualStrategy: true` | `enableDualStrategy: false` |
| **资金分配** | 5000 USDC 单策略 | 2500+2500 USDC 双策略 | 5000 USDC 策略1 |
| **策略数量** | 1个 | 2个智能切换 | 1个 |
| **适应性** | 固定激进策略 | 智能适应波动 | 高频小波动 |
| **交易频率** | 无限制 | 策略1:8次/日, 策略2:5次/日 | 15次/日 |
...
**推荐**：策略1单独运行
```bash
# 设置 dual_strategy_config.json
"enableDualStrategy": false
```
**优势**：稳定的高频小波动策略，风险可控
