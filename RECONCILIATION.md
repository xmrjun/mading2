...
**处理方式**：
- **有买入记录**：使用现有买入均价补充虚拟买单
- **无买入记录**：使用当前市场价格（首次启动场景）
- **无有效价格**：仅同步数量，不增加成本，建议手动核实
...
1. **现有买入统计均价** - 优先使用本地交易记录的平均买入价格
2. **当前市场价格** - 仅在首次启动无买入记录时使用
3. **仅同步数量** - 无法获取有效价格时，只同步数量不增加成本

> **核心原理**：基于本地真实的买入和卖出记录进行对账，避免使用不准确的默认价格
